...
app.post('/api-proxy', async (req, res) => {
  ...
  const expectedHeader = PROXY_HEADER ? PROXY_HEADER.trim() : '';
  if (req.headers['x-app-proxy'] !== expectedHeader) {
    console.log("Header mismatch! Got:", req.headers['x-app-proxy'], "Expected:", expectedHeader);
...
server.on('upgrade', async (request, socket, head) => {
  ...
  if (url.pathname === '/stt-stream') {
    wss.handleUpgrade(request, socket, head, (ws) => {
      console.log('[Node Proxy] STT WebSocket connected');
      ...
      const intervalId = setInterval(async () => {
        if (audioBuffer.length > 0 && activeRequests < 3) {
          activeRequests++;
          // Do not clear the buffer; take a copy to accumulate context
          const processBuffer = Buffer.from(audioBuffer);
          const currentSeq = sequenceNum++;
          
          try {
            const [response] = await speechClient.recognize({
              recognizer: `projects/${GOOGLE_CLOUD_PROJECT}/locations/us/recognizers/_`,
              config: {
                model: 'chirp_3',
                languageCodes: ['en-US'],
            ...
            if (response.results && response.results.length > 0) {
              ...
              if (result.alternatives && result.alternatives.length > 0) {
                ...
                if (transcript.trim() !== '') {
                  ws.send(JSON.stringify({ id: chunkId, seq: currentSeq, text: transcript, isFinal: false }));
                }
              }
            }
          } catch (err) {
            console.error('[Node Proxy] STT Recognize Error:', err.message);
          } finally {
            activeRequests--;
          }

          // If buffer gets too long (e.g., > 5 seconds of audio = 160,000 bytes)
          if (audioBuffer.length > 160000) {
            ws.send(JSON.stringify({ id: chunkId, seq: currentSeq, isFinal: true }));
            audioBuffer = Buffer.alloc(0);
            chunkId = Date.now().toString();
            sequenceNum = 0;
          }
        }
      }, 600); // Flush and recognize every 0.6 seconds
      ...
      ws.on('message', (data) => {
        try {
          ...
          console.error('[Node Proxy] Failed to parse audio chunk', e);
        }
      });
      
      ws.on('close', () => {
        console.log('[Node Proxy] STT WebSocket disconnected');
